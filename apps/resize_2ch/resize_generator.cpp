...
public:
    ...
    void generate() {
        ...
        as_float(x, y) = cast<float>(clamped(x, y));
        ...
        Func resized;
        if (upsample) {
            resized_x(x, y) = sum(kernel_x(x, r) * as_float(r + beginx, y), "resized_x");
            resized_y(x, y) = sum(kernel_y(y, r) * resized_x(x, r + beginy), "resized_y");
            resized = resized_y;
        } else {
            resized_y(x, y) = sum(kernel_y(y, r) * as_float(x, r + beginy), "resized_y");
            resized_x(x, y) = sum(kernel_x(x, r) * resized_y(r + beginx, y), "resized_x");
            resized = resized_x;
        }

        if (input.type().is_float()) {
            output(x, y) = clamp(resized(x, y), 0.0f, 1.0f);
        } else {
            output(x, y) = saturating_cast(input.type(), resized(x, y));
        }
    }
...
HALIDE_REGISTER_GENERATOR(Resize, resize_2ch);
